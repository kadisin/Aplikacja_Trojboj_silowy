...
from database import last_index, check_if_egzist_data_in_database
import Main_Window
...
class Register(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.wzrost = tk.Label(self.master,text="Podaj wzrost[cm]:")
        self.wzrost_entry = tk.Entry(self.master,bd=2)
        ...
        self.wzrost.grid(row=4,column=0,ipady=20)
        self.wzrost_entry.grid(row=4,column=1)
    ...
    def powrot(self):
        self.master.destroy()
        root = tk.Tk()
        self.child = Autoryzacja.Autoryzacja(root)
        self.child.mainloop()
    ...
    def rejestracja(self):
        ...
        self.__wzrost = self.wzrost_entry.get()
        ...
        if(good_data):
            ...
            if(check_if_egzist_data_in_database("SELECT * from user",krotka,[1]) == False):
                ...
                if(u.save_to_database()):
                    ...
                    self.master.destroy()
                    root = tk.Tk()
                    self.child = Main_Window.Main_Window(u,master=root)
                    self.child.mainloop()
